...
**⚠️ КРИТИЧЕСКИ ВАЖНО: Типы моделей**

В AggreGate существует три типа моделей:
1. **Относительная модель** (type=0, по умолчанию) - много экземпляров (по одному на объект), относительные ссылки `{.:var}` в привязках
2. **Абсолютная модель** (type=1) - один экземпляр, абсолютные пути в привязках
3. **Экземплярная модель** (type=2) - создается по требованию

**Для относительной модели ОБЯЗАТЕЛЬНО:**

1. **При создании модели указать параметры:**
...
2. **После создания модели ОБЯЗАТЕЛЬНО:**
   - Создать переменную для хранения данных (`aggregate_create_variable` с `writable: true`)
   - Создать привязку (bindings) для записи данных (`aggregate_set_variable` с переменной `bindings`)
   - Установить выражение пригодности (`validityExpression` через `aggregate_set_variable_field` в `childInfo`)
   - Использовать относительные ссылки в привязках: `{.:variableName}`

**Пример создания абсолютной модели:**
```json
{
  ...
  "parameters": {
    ...
    "description": "Абсолютная модель датчика температуры"
  }
}
```

**Пример создания относительной модели:**
```json
// 1. Создание модели (параметры устанавливаются автоматически)
{
  "tool": "aggregate_create_context",
  "parameters": {
    "parentPath": "users.admin.models",
    "name": "relative_sensor",
    "description": "Относительная модель датчика",
    "modelType": 0,
...
// 2. Создание переменной для хранения данных (ОБЯЗАТЕЛЬНО!)
{
  "tool": "aggregate_create_variable",
  "parameters": {
    "path": "users.admin.models.relative_sensor",
    "variableName": "result",
    "format": "<result><E>",
...
// 3. Создание привязки для записи данных (ОБЯЗАТЕЛЬНО!)
{
...
// 4. Установка выражения пригодности (ОБЯЗАТЕЛЬНО!)
{
...
**⚠️ КРИТИЧЕСКИ ВАЖНО:** Для относительных моделей необходимо выполнить ВСЕ шаги:
1. Создание модели с правильными параметрами
2. Создание переменной (`writable: true`)
3. Создание привязки (bindings) с относительными ссылками
4. Установка выражения пригодности (validityExpression)

Подробнее см. [MCP_RELATIVE_MODEL_COMPLETE_GUIDE.md](MCP_RELATIVE_MODEL_COMPLETE_GUIDE.md)

Подробнее см. [MCP_MODEL_TYPES_GUIDE.md](MCP_MODEL_TYPES_GUIDE.md)
